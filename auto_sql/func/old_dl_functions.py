...
def analysis_page(table, dms_ods_mapping):
    ...
    for rownum in range(6, sheet_rows):
        ...
        odl_field_type = row[4].strip().lower()  # 旧dl字段类型
        dms_table_id = row[6].strip().lower()  # DMS表名
        dms_field_name = row[7].strip()  # DMS字段注释
        dms_field_id = row[8].strip().lower()  # DMS字段名
        dms_field_type = row[10].strip().lower()  # DMS字段类型
        if odl_field_id == '':
            continue
    ...
    odl_table_name = table.cell(4, 2).value  # 旧DL表名称
    ...
    # 解析ods拼接规则
    text = str(table.cell(6, 18)).lower().replace('\\n', ' \\n')
    if "text:" in text:
        text = text[6:-1]
    text_arr = []
    for a in text.split(" "):
