...
def analysis_page(table, dms_ods_mapping):
    ...
    for rownum in range(6, sheet_rows):
        ...
        odl_table_name = row[15].strip().lower()  # 旧DL表名
        if '/' in odl_table_name:   # 应该输出日志提示 zy
            odl_table_id = odl_table_name.split('/')[0]
        elif ',' in odl_table_name:
            odl_table_id = odl_table_name.split(',')[0]
        else:
            odl_table_id = odl_table_name
    ...
    dwd_table_name = table.cell(4, 2).value  # dwd表名称 4是行索引 2是列索引
    ...
    #20220912修改由于中间可能以分号（;）分隔
    if( ';' in table.cell(4, 8).value):
        dms_tables = table.cell(4, 8).value.split(';')  # dms表名称
    ...
    if odl_num == 1:
        ...
        for a in text.split(" "):
            ...
            if a != '':
                text_arr.append(a)
        # # 自己重新获取旧DL表名，以便与join那边的表进行匹配
        # sheet_info_user = {}
        # sheet_rows = table.nrows  # 数据行数
        # if sheet_rows < 6:
        #     sheet_info_user['flag'] = 0
        #     return sheet_info_user
        # sheet_info_user['field_mapping'] = []
        # sheet_info_user['odl_table_short'] = {}
        # gather = set()
        # for rownum in range(6, sheet_rows):
        #     row = table.row_values(rownum)
        #     odl_table = row[15].strip().lower()  # 旧DL表名
        #     if '/' in odl_table:
        #         odl_table = odl_table.split('/')[0]
        #         gather.add(odl_table)
        #     elif ',' in odl_table:
        #         odl_table = odl_table.split(',')[0]
        #         gather.add(odl_table)
        #     else:
        #         continue
        # # 获取旧DL存在多表的表名
        # print("旧DL表名列存在的多表：", gather)
        # for b in text_arr:
        #     if '/' in b:
        #         date_list = b.split('/')
        #         print("date_list的值为：", date_list)
        target_join = " ".join(text_arr)
        # print(target_join)
...
class FixedField:
    def __init__(self):
        self.mapping = {
            ...
            "dwd_del_flag": ", if (a.del_flag=1 , 1, 0) as dwd_del_flag  --dwd删除标识",
...
def dwd_old_sql_create(sheet_info):
    ...
    for row in field_mapping:
        ...
        #修改20220908
        if dwd_field_id == 'load_time':
            row_i = ", trim(NVL(a.load_time, ''))  as " + dwd_field_id + "  --" + dwd_field_name
        elif dwd_field_id == 'operate_type':
            row_i = ", trim(NVL(a.operate_type, 'U'))  as " + dwd_field_id + "  --" + dwd_field_name
        elif dwd_field_id == 'insert_time':
            row_i = ", substr(current_timestamp(), 1, 19) as " + dwd_field_id + "  --" + dwd_field_name
        elif dwd_field_id == 'dwd_del_flag':
            row_i = ", if(a.deleteflag ='1' or a.operate_type='D',1,0) as " + dwd_field_id + "  --" + dwd_field_name
        elif dwd_field_id == 'source_system':
            ...
            if main_type in ('varchar', 'string') and odl_field_main_type in ('varchar', 'string'):
                ...
                #修改20220908
                row_i = ", trim(nvl(" + table_name_short + '.' + odl_field_id + ",'')) as " \
                        + dwd_field_id + "  --" + odl_field_name
                ...
                #        + dwd_field_id + "  --" + odl_field_name
                # 修改20220908
                  row_i = ", nvl(cast(trim(" + table_name_short + '.' + odl_field_id+")" + " as " + field_type + "),0) as " \
                    + dwd_field_id + "  --" + odl_field_name
...
def get_ods_tables(data):
    table = data.sheet_by_name('DWD表目录')  # 通过名称获取sheet内容
...
def create_all(pro):
    ...
    dms_ods_mapping = get_ods_tables(data)   #获取dms表和ods表的映射关系
    # 获取表从第几个开始
