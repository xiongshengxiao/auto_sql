...
class Table:
    ...
    def get_test_row_num_sql(self):
        str_format = '''-- TC-01 整合前后数据条数是否一致:{table_name}
    ...
    def get_test_enumerate_sql(self):
        str_format = '''-- TC-02 枚举转换是否正确:{table_name}
    ...
    def get_test_time_sql(self):
        str_format = '''-- TC-03 日期格式是否正确:{table_name}
    ...
    def get_test_null_sql(self):
        str_format = '''-- TC-04 是否存在null:{table_name}
                    with test_table as ( select * from {table_name} where 1=1)
    ...
    def get_test_space_sql(self):
        str_format = '''-- TC-05 是否存在前后空格:{table_name}
                    with test_table as ( select * from {table_name} where 1=1)
    ...
    def get_test_key_sql(self):
        str_format = '''-- TC-06 业务主键去重:{table_name}
                    with test_table as ( select * from {table_name} where 1=1)
    ...
    def get_test_foreign_key_sql(self):
        str_format = '''-- TC-07 外键在子/维表表中是否都包含:{table_name}
    ...
    def get_test_stagger_sql(self):
        str_format = '''-- TC-08 测试是否有错列:{table_name}
                    with test_table as ( select * from {table_name} where 1=1)
